...
class AccountSummary(Block):
    ...
    def validate(self):
        ...
        tot = sum([self[s] for s in ['Previous Balance', 'Transactions',
...
class TransactionsBlock(Block):
    ...
    def parse(self):
        ...
        if left_col_width == 0:
            self.err(
                "Can't find left column width while parsing %s" %
                self.name)
        ...
        for i in range(len(lines)):
            ...
            for entry in rx['tx_entry'].findall(line):
                ...
                half = lines[i +
                             1][:72] if line.index(entry) < 40 else lines[i +
                                                                          1][72:]
                if half.strip():
                    if not half[:3] in "Jan Feb Mar Apr May Jun Jul Aug Sep Oct Nov Dev":
                        parts[1] += " " + half.strip()
