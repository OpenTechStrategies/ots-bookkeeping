...
class Transaction(transaction.Transaction):
    def as_beancount(self):
        ...
        if self['category'] not in ['Checks Paid', 'Deposits', 'Electronic Deposits',
                                    'Electronic Payments', 'Other Credits', 'Other Withdrawals']:
            raise UnimplementedError( "Can't handle %s in tx dated %s" % (self['category'], self['date']) )
...
class Block(statement.Block):
    ...
    def get_block(self, name=None):
        ...
        if not found_subtotal:
            reg = re.compile(regex_start + "POSTING.*?(\n +Subtotal: +[0-9,.]+)", re.DOTALL | re.MULTILINE)
        ...
        reg = re.compile("^%s[^\n]*?\nPOSTING.*?(\n +Subtotal: +[0-9,.]+)" % name, re.DOTALL | re.MULTILINE)
        m = reg.search(self.text)
        if not m:
            self.err("Couldn't find subtotal for %s section" % name)
        segments += m.group(1)
...
class SubtotalBlock(Block):
        ...
        for i in range(2, len(lines)):
            ...
            if rx['date'].match(line):
                ...
                if i+1 <= len(lines) and not rx['date'].match(lines[i+1]):
                    tx['note'] = " ".join(rx['blanks'].split(lines[i+1].strip())) + " " + tx['note']
    ...
    def validate(self):
        ...
        try:
            account_summary = self.statement.blocks["Account Summary"]['credits'][self.name]
        except KeyError:
            account_summary = self.statement.blocks["Account Summary"]['debits'][self.name]
        ...
        if not (account_summary == abs(subtotal) == abs(total)):
            self.err("Subtotals for %s don't add up.\n" % self.name
...
class AccountSummary(Block):
    ...
    def get_debit_credit_names(self):
        ...
        return ([c for c in self.blocks["Account Summary"]['credits'] if c != "Beginning Balance"]
    ...
    def validate(self):
        ...
        debits = sum([self['debits'][c] for c in self['debits'] if c != "Ending Balance"])
...
class DailyBalance(Block):
    def __init__(self, statement, name):
        self.regex = re.compile("^DAILY BALANCE SUMMARY\n.*?\n\n", re.DOTALL | re.MULTILINE)
        Block.__init__(self, statement, name)
        
    def get_block(self):
        block = "\n".join(["\n".join(block.strip().split("\n")[2:]) for block in self.regex.findall(self.text)])
        return block
    ...
    def validate(self):
        ...
        if not last_daily_bal == end_bal:
            self.err("Last daily balance (%s) != Account Summary ending balance (%s)" % (last_daily_bal, end_bal))
        ...
        running = self.statement.blocks['Account Summary']['credits']['Beginning Balance']
        for tx in self.statement:
            running += tx['amount']
            if tx['date'] in self:
...
class Statement(statement.Statement):
    ...
    def parse(self):
        ...
        blocks_needed = ([c for c in self.blocks["Account Summary"]['credits'] if c != "Beginning Balance"]
                         +[c for c in self.blocks["Account Summary"]['debits'] if c != "Ending Balance"])
        for block in blocks_needed:
            cmd = 'self.blocks["%s"] = %s(self, "%s")' % (block, block.replace(' ',''), block)
            exec(cmd)
